...
const buildPromptText = (days: string): string => `Investigate a cost spike in my Azure subscription over the last ${days} days. I need a root-cause analysis covering:
  ...
  server.registerPrompt(
    ...
    (args) => ({
      messages: [
        {
          role: 'user',
          content: { type: 'text', text: buildPromptText(args.days ?? DEFAULT_DAYS) },
        },
      ],
    }),
  );
